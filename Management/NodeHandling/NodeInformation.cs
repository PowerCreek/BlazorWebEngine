...
namespace BlazorWebEngine.Management.NodeHandling
{
    public class NodeInformation
    {
        ...
        public T AddNodeData<T>(NodeBase node) => AddNodeData<T>(node,(T) Activator.CreateInstance(typeof(T),node));
        private T AddNodeData<T>(NodeBase elem, T data)
        {
            Type mapKey = typeof(T);
            if (!NodeDataMap.ContainsKey(mapKey))
            {
                NodeDataMap.Add(mapKey, new());
            }
            
            if (!NodeDataMap[mapKey].TryAdd(elem.Id, data))
            {
                NodeDataMap[mapKey][elem.Id] = data;
            }
            return data;
        }
        ...
        {
            if (NodeDataMap.TryGetValue(typeof(T), out var map))
            {
                return (T) map[nodeBase.Id];
            }
            return (T) AddNodeData<T>(nodeBase);
        }
    }
}